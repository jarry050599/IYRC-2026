...
No official audio recordings were present in the project assets. `memorial-voice.js` therefore uses the native Web Speech API, currently with rate 0.95, pitch 1 and volume 0.8 (the initial voice release used rate 0.9). Narration follows the supplied Chinese and English scripts; the elapsed year wording follows the existing memorial year calculation. Playback starts only from the voice button's click or keyboard activation. The short paragraphs are queued together inside that gesture; `voiceschanged` only refreshes available choices and never starts or restarts speech.

The existing `lg-lang` switch sets `zh-TW` or `en-US` and cancels the previous narration before resetting the controls. Voice choice prefers an exact locale, then a Traditional Chinese locale where available, then the base language, and finally the browser default with the requested utterance locale. Within each match, available quality-label hints are preferred, followed by the system default and a local voice; no specific voice name is assumed. The application makes no external speech-service or API-key integration. Actual voice quality and availability depend on the browser and installed operating-system voices.
