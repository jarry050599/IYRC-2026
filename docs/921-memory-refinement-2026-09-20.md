...
No official audio recordings were present in the project assets. `memorial-voice.js` therefore uses the native Web Speech API, currently with rate 0.93 for a recognized male voice or 0.95 for a fallback voice, pitch 1 and volume 0.8 (the initial voice release used rate 0.9). Narration follows the supplied Chinese and English scripts; the elapsed year wording follows the existing memorial year calculation. Playback starts only from the voice button's click or keyboard activation. The short paragraphs are queued together inside that gesture; `voiceschanged` only refreshes available choices and never starts or restarts speech.

The existing `lg-lang` switch sets `zh-TW` or `en-US` and cancels the previous narration before resetting the controls. Voice choice prefers an exact locale, then a Traditional Chinese locale where available, then the base language, and finally the browser default with the requested utterance locale. Within each match, recognized male voices are preferred, then quality-label hints, the system default and local availability; no specific voice is assumed to be installed. The application makes no external speech-service or API-key integration. Actual voice quality and availability depend on the browser and installed operating-system voices.
...
## Narration selection and phrasing refinement

To address mechanical delivery, the player no longer unconditionally takes the first local voice in a matching locale. It ranks optional `Premium`, `Natural`, `Neural` and `Enhanced` hints in the names/URIs of voices actually returned by the browser, then the system default, then local availability. These labels are a heuristic, not a standardized quality property or a listening assessment. Locale matching still wins: a higher-ranked voice in another language or region does not displace an available exact match. Missing names and empty voice lists retain the existing safe fallback.

Rate changes from 0.9 to 0.95 to stay closer to the selected voice's normal pace; pitch remains 1. The Chinese speech-only text spells the date as `一九九九年九月二十一日`, removes pauses after `我們` and `能不能`, and ends the final question with a question mark. The words and meaning remain, and visible page text is unchanged. Paragraph queueing, manual start, language switching, pause/resume/replay, cancellation and every-load introduction behavior are unchanged. No additional control, sound effect, music, audio service or generated recording was added.

The module advances to `memorial-voice.js?v=921-2`, with service-worker cache v36. All sixteen adapter tests and 46 Chromium interaction/layout checks pass, including selection/fallback, the revised spoken text and the existing speech lifecycle. No runtime/resource errors or automated accessibility violations were found in the checked states. Perceived naturalness cannot be certified with mocked speech events or a headless engine without installed voices. A device with only its original basic voice may retain the same timbre; consistently natural, directed narration would require approved Chinese and English audio recordings.

## Calm male narration preference

The user's requested male-voice trial now prefers recognized male voices within each existing locale tier. It accepts explicit `male` / `男聲` labels or male URI tokens, and a conservative list from Microsoft's published voice catalogues, including Taiwanese Mandarin Zhiwei/YunJhe and English David/Mark/Guy. It checks only the actual browser voice list; catalogued voices are never treated as installed. Female markers take precedence over a conflicting name hint, and unknown voices are not reported as confirmed male. Exact-locale matching remains ahead of gender preference, so an available Taiwanese voice is not replaced with an English or different-region voice merely to obtain a male timbre.

Recognized male voices use rate 0.93 with their natural pitch of 1. The fallback keeps rate 0.95 and pitch 1; no pitch-shift is used to imitate a male voice. During playback, the existing status caption says `男聲旁白播放中` / `Male narration playing` when selection is recognized. Otherwise it says `未找到男聲，正使用裝置音色播放` / `Playing · Male voice not found`. The latter means no recognized male voice was found in the selected locale tier, not that the operating system can never provide one. No new buttons, overlay, autoplay, service integration or recordings are introduced.

The optional adapter state callback may now include `{male: boolean}` selection metadata; adapters that omit it continue to use the original playback-status wording. Metadata clears on stop and language changes, and asynchronous voice updates affect only the next user-initiated play.

Validation: all 21 adapter tests and 51 Chromium interaction/layout checks pass. These include same-locale male preference, female/unknown/empty-list fallbacks, normal pitch, Chinese/English status labels, delayed voices, cancellation, keyboard controls, repeated entrance and a compact mobile fallback layout. No runtime/resource errors or automated accessibility violations were found in the checked states. Existing visible page copy, overlay markup and every-load behavior are unchanged. Script version: `memorial-voice.js?v=921-3`; service-worker cache: v37. Actual audible timbre on the user's device has not been verified; the existing physical Safari/Android limitations still apply.

Catalogue references: [Microsoft Narrator voices](https://support.microsoft.com/en-us/accessibility/windows/narrator/appendix-a-supported-languages-and-voices), [Microsoft speech voice catalogue](https://learn.microsoft.com/en-us/azure/ai-services/speech-service/language-support?tabs=tts). The [Web Speech voice interface](https://developer.mozilla.org/en-US/docs/Web/API/SpeechSynthesisVoice) exposes no standardized gender field; recognition is deliberately conservative.
